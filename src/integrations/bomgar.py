	...
	def addSession(self, thisSessionId:int, thisQueueId:int, startTime:int):
		...
		if self.__sessionExists__(thisSessionId):
			...
			if queueId == thisQueueId:
				self.currentSessions.append({"id": thisSessionId, "startTime": datetime.fromtimestamp(startTime)})
				return True
			return False
	...
	for table in root:
		if table == "support_session":
			for session in root[table]:
				...
				currentSessions.addSession(session, root[table][session]["queue_id"], root[table][session]["queue_entry_timestamp"])
...
def updateBearerToken(func):
	def wrap(*args, **kwargs):
		global bearerToken
		bearerToken = (json.loads(requests.post("https://bomgar-app.tamu.edu/oauth2/token?grant_type=client_credentials", auth=requests.auth.HTTPBasicAuth(secrets.bomgar.username, secrets.bomgar.password)).text)["access_token"])
...
@updateBearerToken
def on_open(ws):
	auth = ("""{"type" : "authenticate","credentials" :{"bearer_token" : \"""" + bearerToken + """\"}}\n""")
	ws.send(auth)
	ws.send("""{"type" : "subscribe","tables" : ["support_session", "queue"]}\n""")
